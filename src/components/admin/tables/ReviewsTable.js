        ...
        <div className="admin-panel"style={{ paddingTop: '50px',paddingBottom: '250px'}}>
            ...
            {reviews.map(review => (
                <tr key={review.id}>
                    ...
                    <td>
                        <button onClick={() => deleteReview(review.id)} className="button-delete"style={{ background: 'salmon', color: 'white'}}>УДАЛИТЬ</button>
                        <button onClick={() => approveReview(review.id)} className="button-approve"style={{ background: 'lightgreen', color: 'white'}}>УТВЕРДИТЬ</button>
                    </td>
                </tr>
            ))}
            </tbody>
