        ...
        <div className="admin-panel"style={{ paddingTop: '50px',paddingBottom: '250px'}}>
            ...
            {users.map(user => (
                <tr key={user.id}>
                    ...
                    <td>{user.role}</td>
                    <td>
                        <button onClick={() => navigateToEditUser(user.id)} className="button-edit" style={styles.buttone}>РЕДАКТИРОВАТЬ</button>
                        <button onClick={() => deleteUser(user.id)} className="button-delete"style={{ background: 'salmon', color: 'white'}}>УДАЛИТЬ</button>
                        <button onClick={() => navigateToUserBookings(user.id)} className="button-view"style={styles.buttonb}>ПОСМОТРЕТЬ БРОНИРОВАНИЕ</button>
                    </td>
                </tr>
            ))}
            </tbody>
