        ...
        <div className="admin-panel"style={{ paddingTop: '50px',paddingBottom: '250px'}}>
            ...
            {categories.map(category => (
                <tr key={category.id}>
                    ...
                    <td>{category.name}</td>
                    <td>
                        <button onClick={() => deleteCategory(category.id)} className="button-delete"style={{ background: 'salmon', color: 'white'}}>УДАЛИТЬ</button>
                        <button onClick={() => navigateToEditCategory(category.id)} className="button-edit" style={styles.buttone}>РЕДАКТИРОВАТЬ</button>
                    </td>
                </tr>
            ))}
            </tbody>
